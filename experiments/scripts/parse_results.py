...
def build_capacity_matrix(rows: List[Dict[str, str]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        if row.get("benchmark") != "probe":
            continue
        key = (row["platform"], row["backend"], row["deployment"])
    ...
    for (platform, backend, deployment), row in sorted(grouped.items()):
        out.append(
            {
                "platform": platform,
                "backend": backend,
...
def build_perf_matrix(rows: List[Dict[str, str]]) -> List[Dict[str, object]]:
    qft_values: Dict[Tuple[str, str, str, str, int], List[float]] = defaultdict(list)
    random_values: Dict[Tuple[str, str, str, str, int], List[float]] = defaultdict(list)
    h_rep_values: Dict[Tuple[str, str, str, str, int, str], List[float]] = defaultdict(list)
    ...
    for row in rows:
        ...
        if benchmark == "qft" and row.get("stage_label") == "total":
            ...
            if value is not None:
                qft_values[(platform, backend, deployment, benchmark, qubits)].append(value)
        elif benchmark == "random":
            value = float_or_none(row.get("total_time_s"))
            if value is not None:
                random_values[(platform, backend, deployment, benchmark, qubits)].append(value)
        elif benchmark == "h_sweep":
            value = float_or_none(row.get("gate_time_s"))
            rep = row.get("rep")
            if value is not None and rep is not None:
                h_rep_values[(platform, backend, deployment, benchmark, qubits, rep)].append(value)

    perf_rows: List[Dict[str, object]] = []
    for key, values in sorted(qft_values.items()):
        mean_value, std_value, sample_count = grouped_stats(values)
        platform, backend, deployment, benchmark, qubits = key
        perf_rows.append(
            {
                "platform": platform,
                "backend": backend,
    ...
    h_grouped: Dict[Tuple[str, str, str, str, int], List[float]] = defaultdict(list)
    for key, values in sorted(h_rep_values.items()):
        platform, backend, deployment, benchmark, qubits, _rep = key
        h_grouped[(platform, backend, deployment, benchmark, qubits)].append(statistics.fmean(values))
...
def build_degradation_matrix(perf_rows: List[Dict[str, object]]) -> List[Dict[str, object]]:
    ...
    for row in perf_rows:
        key = (str(row["platform"]), str(row["backend"]), str(row["deployment"]), str(row["benchmark"]))
...
def build_qft_stage_matrix(rows: List[Dict[str, str]]) -> List[Dict[str, object]]:
    grouped: Dict[Tuple[str, str, str, int, int, str], List[float]] = defaultdict(list)
    for row in rows:
        ...
        if value is None or qubits is None or stage is None:
            continue
        key = (row["platform"], row["backend"], row["deployment"], qubits, stage, row["stage_label"])
        grouped[key].append(value)

    out: List[Dict[str, object]] = []
    for key, values in sorted(grouped.items()):
        platform, backend, deployment, qubits, stage, stage_label = key
        mean_value, std_value, sample_count = grouped_stats(values)
        out.append(
            {
                "platform": platform,
...
def build_h_target_matrix(rows: List[Dict[str, str]]) -> List[Dict[str, object]]:
    ...
    for row in rows:
        ...
        if value is None or qubits is None or target is None:
            continue
        key = (row["platform"], row["backend"], row["deployment"], qubits, target)
        grouped[key].append(value)

    out: List[Dict[str, object]] = []
    for key, values in sorted(grouped.items()):
        platform, backend, deployment, qubits, target = key
        mean_value, std_value, sample_count = grouped_stats(values)
        out.append(
            {
                "platform": platform,
...
def build_mpi_extension_matrix(rows: List[Dict[str, str]]) -> List[Dict[str, object]]:
    grouped: Dict[Tuple[str, str, str, int, int, str], List[float]] = defaultdict(list)
    counts: Dict[Tuple[str, str, str, int, int, str], int] = defaultdict(int)
    ...
    for row in rows:
        ...
        key = (row["platform"], row["backend"], row["deployment"], qubits, num_nodes, status)
        counts[key] += 1
        value = float_or_none(row.get("total_time_s"))
        if value is not None:
            grouped[key].append(value)
    ...
    for key in keys:
        platform, backend, deployment, qubits, num_nodes, status = key
...
def main() -> int:
    ...
    write_tsv(
        ...
        ["platform", "backend", "deployment", "max_qubits", "probe_attempts", "alloc_time_s", "validation_time_s", "status"],
        capacity_rows,
    )
    write_tsv(
        out_dir / "perf_matrix.tsv",
        ["platform", "backend", "deployment", "benchmark", "num_qubits", "mean_time_s", "std_time_s", "samples"],
        perf_rows,
    )
    write_tsv(
        out_dir / "degradation_matrix.tsv",
        ["platform", "backend", "deployment", "benchmark", "num_qubits", "baseline_qubits", "baseline_mean_time_s", "mean_time_s", "slowdown_vs_26"],
        degradation_rows,
    )
    write_tsv(
        out_dir / "qft_stage_matrix.tsv",
        ["platform", "backend", "deployment", "num_qubits", "stage", "stage_label", "mean_stage_time_s", "std_stage_time_s", "samples"],
        qft_stage_rows,
    )
    write_tsv(
        out_dir / "h_target_matrix.tsv",
        ["platform", "backend", "deployment", "num_qubits", "target_qubit", "mean_gate_time_s", "std_gate_time_s", "samples"],
        h_target_rows,
    )
    write_tsv(
        out_dir / "mpi_extension_matrix.tsv",
        ["platform", "backend", "deployment", "num_qubits", "env_num_nodes", "status", "mean_total_time_s", "std_total_time_s", "samples", "rows"],
        mpi_rows,
    )
