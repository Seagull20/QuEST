#!/usr/bin/env bash
#SBATCH --job-name=quest-thread-point
...
resolve_qubit() {
    ...
    [ -n "${SLURM_ARRAY_TASK_ID:-}" ] || die "SLURM_ARRAY_TASK_ID missing for manifest-driven thread sweep."
...
BENCH_THREADS="${BENCH_THREADS:-32}"
...
case "${BENCHMARK}" in
    qft)
        OUTPUT_PATH="${RUN_RAW_DIR}/qft_archer2_${BACKEND}_${DEPLOYMENT}_q${QUBIT}_t${BENCH_THREADS}.tsv"
        CMD=(
            "${EXE}"
            --qubits "${QUBIT}"
            --reps "${REPS}"
        ...
        ;;
    h_sweep)
        OUTPUT_PATH="${RUN_RAW_DIR}/h_sweep_archer2_${BACKEND}_${DEPLOYMENT}_q${QUBIT}_t${BENCH_THREADS}.tsv"
        CMD=(
            "${EXE}"
            --qubits "${QUBIT}"
            --reps "${REPS}"
    ...
    random)
        ...
        OUTPUT_PATH="${RUN_RAW_DIR}/random_archer2_${BACKEND}_${DEPLOYMENT}_q${QUBIT}_t${BENCH_THREADS}.tsv"
        CMD=(
            "${EXE}"
            --qubits "${QUBIT}"
            --depth "${DEPTH}"
...
info "ARCHER2 thread-sweep point job"
info "Benchmark: ${BENCHMARK}"
info "Qubit: ${QUBIT}"
info "Threads: ${BENCH_THREADS}"
info "Node: $(hostname)"
